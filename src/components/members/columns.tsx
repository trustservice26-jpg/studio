...
export const columns: ColumnDef<Member>[] = [
  ...
  },
  {
    ...
    header: () => {
        const { language } = useAppContext();
        return <div className="hidden md:table-cell">{language === 'bn' ? 'ফোন' : 'Phone'}</div>
    },
     cell: ({ row }) => {
        const phone = row.getValue("phone") as string;
        return <div className="hidden md:table-cell">{phone}</div>
    }
  },
  {
    accessorKey: "email",
    header: () => {
        const { language } = useAppContext();
        return <div className="hidden lg:table-cell items-center"><Mail className="mr-2 h-4 w-4" />{language === 'bn' ? 'ইমেইল' : 'Email'}</div>
    },
    cell: ({ row }) => {
        const email = row.getValue("email") as string;
        return <div className="hidden lg:table-cell">{email ? <a href={`mailto:${email}`} className="hover:underline">{email}</a> : <span className="text-muted-foreground/50">N/A</span>}</div>
    }
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const { language } = useAppContext();
      return <div className="hidden xl:table-cell">{date.toLocaleDateString(language === 'bn' ? 'bn-BD' : 'en-US')}</div>
    },
  },
  {
    id: "actions",
