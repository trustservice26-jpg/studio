...
export function PdfDocument({ member, language, isRegistration }: PdfDocumentProps) {
    ...
    return (
        <div style={{ width: '800px', padding: '20px', fontFamily: 'sans-serif', color: '#000', background: '#fff' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '15px', marginBottom: '25px' }}>
                <tbody>
                    {member.joinDate && <tr style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '10px 0', fontWeight: 'bold' }}>{language === 'bn' ? 'যোগদানের তারিখ' : 'Joining Date'}</td><td style={{ padding: '10px 0', textAlign: 'right' }}>{new Date(member.joinDate).toLocaleDateString(language === 'bn' ? 'bn-BD' : 'en-US')}</td></tr>}
                    {member.dob && <tr style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '10px 0', fontWeight: 'bold' }}>{language === 'bn' ? 'জন্ম তারিখ' : 'Date of Birth'}</td><td style={{ padding: '10px 0', textAlign: 'right' }}>{member.dob}</td></tr>}
                    {member.fatherName && <tr style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '10px 0', fontWeight: 'bold' }}>{language === 'bn' ? 'পিতার নাম' : "Father's Name"}</td><td style={{ padding: '10px 0', textAlign: 'right' }}>{member.fatherName}</td></tr>}
                    ...
                    <tr>
                        ...
                        <td style={{ padding: '10px 0', textAlign: 'right' }}>
                            {member.address ? <div style={{ minHeight: '56px', whiteSpace: 'pre-wrap' }}>{member.address}</div> : (
                                <>
                                    <div style={{borderBottom: '1px solid #999', height: '22px', marginBottom: '12px'}}></div>
                                    <div style={{borderBottom: '1px solid #999', height: '22px'}}></div>
                                </>
                            )}
                        </td>
                    </tr>
            ...
            <div style={{marginTop: '20px'}}>
                <h3 style={{fontSize: '16px', fontWeight: 'bold', borderBottom: '1px solid #ccc', paddingBottom: '5px', marginBottom: '15px'}}>{language === 'bn' ? 'শর্তাবলী' : 'Conditions'}</h3>
                <ul style={{ listStyleType: 'none', paddingLeft: '0px', fontSize: '11px', color: '#333' }}>
                    {conditions.map((condition, index) => (
                        <li key={index} style={{ marginBottom: '6px', fontWeight: 'bold', textIndent: '-1em', paddingLeft: '1em' }}>• {condition}</li>
                    ))}
                </ul>
            </div>

            <div style={{ marginTop: '60px', paddingTop: '15px', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ borderTop: '2px dotted #aaa', width: '180px', margin: '30px 0 5px 0' }}></div>
             ...
             <div style={{ marginTop: '25px', fontSize: '10px', color: '#555', textAlign: 'center', paddingTop: '15px', borderTop: '1px solid #eee' }}>
                ...
                <p style={{ fontWeight: 'bold', fontStyle: 'italic', marginTop: '10px' }}>DEVELOPED BY (supported) AL-SADEEQ TEAM</p>
             </div>
        </div>
    );
}
